...
cmd deny 'kill -9 -1'
cmd deny 'docker compose down -v'
cmd deny 'docker compose down --volumes'
cmd deny 'docker-compose down -v'
cmd deny 'docker compose -f docker-compose.yml down --remove-orphans -v'
cmd deny 'docker volume rm cardoso-sound-api_pgdata'
cmd deny 'docker volume prune -f'
cmd deny 'docker system prune -a --volumes'
cmd deny 'rm -rf docker-compose.yml'
cmd deny 'rm -f package.json'
cmd deny 'rm -r docs'
cmd deny 'rm -rf dist'
cmd deny 'rm -rf node_modules'
cmd deny 'rm docker-compose.yml'
cmd deny 'ls && rm docker-compose.yml'
cmd deny '/bin/rm docker-compose.yml'
cmd deny '\rm docker-compose.yml'
cmd deny 'bash -c "rm docker-compose.yml"'
cmd deny 'ls *.yml | xargs rm'
cmd deny 'rmdir docs/old'
cmd deny 'unlink docker-compose.yml'
cmd deny 'git rm docker-compose.yml'
cmd deny 'git rm --cached docker-compose.yml'
cmd deny 'mv docker-compose.yml /tmp/'
cmd deny 'git mv docker-compose.yml compose.yml'
cmd deny 'truncate -s 0 docker-compose.yml'
cmd deny '> docker-compose.yml'
cmd deny ': > docker-compose.yml'
cmd deny 'ls; >docker-compose.yml'
cmd deny 'node -e "require(\"fs\").rmSync(\"docker-compose.yml\")"'
cmd deny 'node -e "fs.unlinkSync(x)"'
cmd deny 'python3 -c "import os; os.remove(\"docker-compose.yml\")"'
cmd deny 'python3 -c "import shutil; shutil.rmtree(\"docs\")"'
cmd deny 'perl -e "unlink(\"docker-compose.yml\")"'
cmd deny 'find . -name "*.log" -delete'
cmd deny 'find . -name "*.yml" -exec rm {} +'
cmd deny 'rsync -a --delete empty/ docs/'
cmd deny 'git clean -fdx'
cmd deny 'git clean -f'
...
cmd allow 'docker compose down'
cmd allow 'docker compose logs -f db'
cmd allow 'docker volume ls'
...
cmd allow 'docker run --rm postgres:17-alpine pg_isready'
cmd allow 'curl -X POST http://localhost:3333/api/auth/unlink-account'
cmd allow 'git log --format=%h docs/specs/04-autenticacao-e-seguranca.md'
cmd allow 'pnpm test > /tmp/test.log 2>&1'
cmd allow 'pnpm test &> /tmp/test.log'
cmd allow 'cat > notes.md <<EOF'
cmd allow 'pnpm format && git diff --stat'
cmd allow 'cp .env.example .env.test'
